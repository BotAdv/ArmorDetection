...
//这个文件定义了一个名为ArmorDetectorNode的ROS 2节点,
//用于从图像中检测装甲板(armors),并计算其在机器人坐标系中的位置。
//它还提供了调试功能,例如发布检测结果的可视化标记(markers)和中间图像。
...
ArmorDetectorNode::ArmorDetectorNode(const rclcpp::NodeOptions &options)
  ...
  // Detector 初始化检测器对象，用于装甲板检测
  detector_ = initDetector();

  // Tricks to make pose more accurate 启用Bundle Adjustment（BA）优化检测结果
  use_ba_ = this->declare_parameter("use_ba", true);

  // Armors Publisher 创建发布者，用于发布检测到的装甲板信息
  armors_pub_ = this->create_publisher<rm_interfaces::msg::Armors>(
      "armor_detector/armors", rclcpp::SensorDataQoS());

  // Transform initialize ，声明参数target_frame为目标坐标系
  ...
  //创建发布者marker_pub_,用于发布可视化标记
  marker_pub_ = this->create_publisher<visualization_msgs::msg::MarkerArray>(
      "armor_detector/marker", 10);

  // Debug Publishers 是否启用调试功能
...
void ArmorDetectorNode::imageCallback(
  ...
  try {
    // 尝试查询从odom_frame_到图像消息frame_id的坐标系变换关系
  ...
  // Detect armors 检测图像中的装甲板
  auto armors = detectArmors(img_msg);

  // Init message 初始化装甲板消息的头部信息并清空之前的检测结果
  armors_msg_.header = img_msg->header;
  armors_msg_.armors.clear();

  // Extract armor poses 位姿估计信息存在，则计算其在机器人坐标系下位置
  if (armor_pose_estimator_ != nullptr) {
    armors_msg_.armors =
        armor_pose_estimator_->extractArmorPoses(armors, imu_to_camera_);

    // std::string path =
    //   fmt::format("/home/sunrise/fyt2024-log/images/{}/{}.jpg",
  ...
  // Publishing detected armors 发布检测结果
...
std::vector<Armor> ArmorDetectorNode::detectArmors(
    const sensor_msgs::msg::Image::ConstSharedPtr &img_msg) {
  // Convert ROS img to cv::Mat  将ROS图像消息转化为opencv图像
...
// Register the component with class_loader.注册节点
